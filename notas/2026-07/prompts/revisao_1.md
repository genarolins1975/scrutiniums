...
[INFERÊNCIA] Esta leitura parte da alta de {{taxa_pf.delta_12m_pp|abs}} da taxa das pessoas físicas em doze meses, da alta de {{inad_pf.delta_12m_pp|abs}} da inadimplência das pessoas físicas em doze meses e da alta de {{comprometimento.delta_12m_pp|abs}} do comprometimento de renda das famílias em doze meses, este com data de referência {{data:comprometimento.nivel}}. O custo do crédito, a inadimplência e o comprometimento de renda das pessoas físicas estão acima do nível de doze meses antes. Isso indica menor folga das famílias para o serviço da dívida do que doze meses antes. Há fatos do pacote em outro sentido, que ficam registrados: a queda de {{taxa_pf.delta_mes_pp|abs}} da taxa das pessoas físicas no mês e a queda de {{endividamento.delta_mes_pp|abs}} do endividamento das famílias no mês, este com data de referência {{data:endividamento.nivel}}. Nenhum deles altera a comparação em doze meses em que a leitura se apoia. Refutaria esta leitura: variação em doze meses nula ou negativa do comprometimento de renda das famílias ou da inadimplência das pessoas físicas nos dados vigentes.

[INFERÊNCIA] Esta leitura parte da queda de {{saldo_pj.var_mes_pct|abs}} do saldo às pessoas jurídicas no mês e da alta de {{taxa_pj.delta_mes_pp|abs}} da taxa e de {{spread_pj.delta_mes_pp|abs}} do spread do mesmo segmento no mês, enquanto a taxa total teve queda de {{taxa_total.delta_mes_pp|abs}} e o spread total teve queda de {{spread_total.delta_mes_pp|abs}} no mês. Só na taxa e no spread o segmento foi em sentido oposto ao do agregado. Essa combinação é compatível com condições de crédito às pessoas jurídicas menos favoráveis ao tomador no mês. O agregado reúne pessoas físicas e jurídicas, e parte da diferença reflete a queda da taxa e do spread das pessoas físicas no mesmo período. A inadimplência fica fora desta leitura sobre condições de crédito: a das pessoas jurídicas teve alta de {{inad_pj.delta_mes_pp|abs}} no mês, e a total também teve alta de {{inad_total.delta_mes_pp|abs}} no mês. A leitura vale só para a variação do mês. Em doze meses, o mesmo segmento mostra queda de {{taxa_pj.delta_12m_pp|abs}} da taxa, queda de {{spread_pj.delta_12m_pp|abs}} do spread e alta real de {{concessoes_pj.var_12m_real_pct|abs}} das concessões, fatos que não acompanham a leitura mensal. A leitura não é unânime: uma leitura independente dos mesmos fatos atribui ao segmento das pessoas jurídicas condições menos favoráveis sem restringir a conclusão ao mês. Refutaria esta leitura: variação mensal das concessões às pessoas jurídicas com alta, dado que não está no pacote, ou evidência de que a queda do saldo decorre de liquidação de operações e não de menor contratação.
...
Validador mecânico:
M6 linha 28: spread_pj.delta_mes_pp citado sem sinal e sem palavra de direção
M11 linha 26: o texto antes de inad_pf.delta_12m_pp fala de taxa, não de inad
M11 linha 26: o texto antes de comprometimento.delta_12m_pp fala de inad, não de comprometimento
M11 linha 26: o texto antes de endividamento.delta_mes_pp fala de taxa, não de endividamento
M11 linha 28: o texto antes de taxa_pj.delta_mes_pp fala de saldo, não de taxa
M11 linha 28: o texto antes de spread_pj.delta_mes_pp fala de taxa, não de spread
M11 linha 28: o texto antes de spread_pj.delta_12m_pp fala de taxa, não de spread
M11 linha 28: o texto antes de concessoes_pj.var_12m_real_pct fala de spread, não de concessoes
