...
Conteúdo de cada ciclo: `pacote.json` (fatos e hashes da gold), `prompts/` e `saidas/` por papel e por rodada, `uso/` (tokens e modelo que respondeu), `validacao_mecanica_<rodada>.json` (e `validacao_mecanica.json`, a última), `nota.md` (fonte com marcadores), `nota_final.md` (texto renderizado da última versão que passou no validador mecânico; se vai a público decide `decisao.json`), `manifesto.json` (reprodutibilidade), `decisao.json` (decisão, rodadas e parecer de cada revisor), `metricas.json`, e depois da auditoria mensal `auditoria.json` e, se houver erro, `errata.md`.
...
O ciclo `2026-07` foi o primeiro no formato da constituição v2 e foi rejeitado (ver `docs/AVALIACAO_AGENTES_2026-09-24.md`, seção 9).
