...
- Status no painel: **INTEGRADA** (desde 06/08/2026). Coletor `pipeline/sources/epae.py` baixa a planilha oficial, guarda o bronze com hash e grava os fluxos em `epae_fluxo` (silver); `pipeline/epae.py` publica `epae.json` com a série mensal dos dois sentidos (pessoas → seção e seção → pessoas), o líquido (derivação declarada), o número de transações e o total PF→PJ como escala. A aba de bets exibe a série como DADO OBSERVADO da seção inteira, sem atribuir parcela às apostas, e confronta o líquido observado de 2025 com o valor atribuído pelo estudo Comsefaz/Cicef — a diferença entre os dois é o contrafactual, e fica explícita na tela.
- Atualização automática: SIM (pipeline diário; o BC republica o arquivo inteiro a cada mês e revisa os quatro últimos meses) · Confiabilidade: A (dado administrativo), com a limitação setorial acima
