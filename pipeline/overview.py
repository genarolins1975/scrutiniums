...
INDICATOR_DICTIONARY = [
    ...
    {"nome": "Score de estresse setorial", "definicao": "Posição do setor no espectro de estresse (0 = mínimo, 100 = máximo).",
     "formula": "50 + 20 × Σ(peso × z) — atividade 0,45; condições de crédito 0,20; estresse empresarial 0,20; capacidade financeira 0,15",
...
CHANGELOG = [
    {"versao": "0.17.0", "data": "2026-07-28", "mudancas": [
        ...
        "Quatro índices separados (adoção, utilização, conversão, qualidade técnica) com componentes, pesos, cobertura e confiança — conversão declarada INDISPONÍVEL (funil não publicado) em vez de estimada; substituem qualquer índice único de maturidade.",
    ...
    {"versao": "0.12.0", "data": "2026-07-28", "mudancas": [
        ...
        "Índice de maturidade demo substituído por 'maturidade estrutural' honesta (escala de chamadas + cobertura de famílias de API) — conversão/latência por organização não são públicas e o índice antigo foi descartado em vez de simulado.",
    ]},
...
SCORE_CARDS = [
    ...
    {"nome": "Estresse setorial",
     "componentes": ["atividade 0,45", "condições de crédito 0,20", "estresse empresarial 0,20 (DEMO)", "capacidade financeira 0,15 (DEMO)"],
    ...
    {"nome": "Maturidade estrutural Open Finance",
     "componentes": ["escala de chamadas (relativa ao líder)", "cobertura de famílias de API"],
     "pesos": "iguais", "normalizacao": "min-max no grupo divulgado",
     "tratamento_ausencia": "sem match no diretório → dimensão omitida",
     "cobertura": "top-20 de chamadas divulgado", "sensibilidade": "—",
     "validacao": "NÃO mede conversão/latência (não públicas); índice demo anterior foi aposentado", "versao": "0.12.0"},
]
