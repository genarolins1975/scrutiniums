...
/** Verificação do código de e-mail (etapa 1). O e-mail pendente vive em cookie httpOnly. */
export function VerificarEmailForm({ maskedEmail }: { maskedEmail: string }) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/onboarding/email/check", {
        method: "POST",
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="flex flex-col gap-6">
      <p className="text-sm text-carvao-muted">
        Enviamos um código de 6 dígitos para <span className="font-medium text-carvao">{maskedEmail}</span>.
      </p>
